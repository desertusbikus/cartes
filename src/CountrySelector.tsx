...
import { COUNTRIES, SelectMenuOption } from "./countries_utils/countries";
...
export interface CountrySelectorProps {
  id: string;
  disabled?: boolean;
  onCountryChange?: (value: SelectMenuOption) => void;
...
export default function CountrySelector({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = ({ target }: MouseEvent) => {
      if (
        mutableRef.current &&
        !mutableRef.current.contains(target as Node) &&
        open
      ) {
        onToggle?.();
        setQuery("");
  ...
  return (
    <div ref={ref} className="w-3/4">
      <div className="mt-1 relative">
        <button
          type="button"
          className={classNames(
            // formInputClasses,
            "relative w-full border border-gray-600 shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm",
            {
